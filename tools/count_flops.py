...
def parse_args():
    ...
    parser.add_argument('work_dir', help='train config file path')
    parser.add_argument('--only_backbone_flops',action="store_true",help='whether only flops of backbone')
    parser.add_argument(
...
def main():
    ...
    for n, model_meta in enumerate(metas_per_rank):
        ...
        flops, params = get_model_complexity_info(
            ...
            as_strings=args.as_strings, only_backbone_flops=args.only_backbone_flops)
        #pdb.set_trace()
        #simulate_input = torch.rand(1,3,2048,512).cuda()
        #flops = FlopCountAnalysis(model, simulate_input) 我们的算出来略微小一点
        #print(flops.total())
