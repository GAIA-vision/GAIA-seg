...
from gaiavision.core.ops import ElasticConv2d, ElasticLinear, ElasticLayerNorm
...
class ElasticTransformerEncoderLayer(nn.Module, DynamicMixin):
    def __init__(self,
        ...
        self.attn = ElasticMHA(embed_dim=embed_dim,num_heads=num_heads,proj_drop=0.0,attn_drop=0.0,drop_path=drop_path)
...
class ElasticEncoder(nn.ModuleList, DynamicMixin):
    ...
    def manipulate_num_heads(self, arch_meta):
        self.num_heads_state = arch_meta
        for m in self:
            m.attn.manipulate_arch(arch_meta)
    ...
    def forward(self, x, extra_out_indices=None):
        if getattr(self, '_deploying', False):
            return self.deploy_forward(x)
        outputs = []
        for i in range(self.num_layers_state):
            x = self[i](x)
            if extra_out_indices is not None:
                if i in extra_out_indices:
                    outputs.append(x)
        if extra_out_indices is not None:
            outputs.append(x)
            return outputs
...
class ElasticTransformer(BaseBackbone, DynamicMixin):
    ...
    def __init__(self,
        ...
        super(ElasticTransformer, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, x):
        ...
        if cls_tokens is not None:
            for idx in range(len(outputs)):
                ...
                outputs[idx] = outputs[idx].reshape(B,H//self.patch_size,W//self.patch_size,-1)
                outputs[idx] = outputs[idx].permute(0,3,1,2).contiguous() # [N,C,H,W]
        
        # x = self.norm1(x) # 这个地方得check一下，对于分割，检测这种多输出的，是每个输出
        # 都要经过这个norm 吗？ 这个地方感觉设计的很奇怪，本来额外加一层layer norm是针对
        # 分类设计的，mmseg中也没有对这个作处理，感觉可以试验一下，是不是在加layer norm的时候
        # 都加这一个其实好一点。。。，但是这个应该不共享参数？
